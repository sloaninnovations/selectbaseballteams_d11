...
class SchemaConfigListenerWebTest extends BrowserTestBase {
  ...
  public function testConfigSchemaChecker(): void {
    ...
    try {
      ...
      $this->assertEquals("Schema errors for config_test.types with the following errors: config config_test.types, property array: variable type is integer but applied schema class is Drupal\Core\Config\Schema\Sequence, config config_test.types, property foo: missing schema, 0 [foo] &#039;foo&#039; is not a supported key.", $e->getMessage());
    }
